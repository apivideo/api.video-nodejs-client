...
export class VideosApi {
    ...
    public async getStatus(videoId: string): Promise<VideoStatus > {
		...
		// Path Params
    	const localVarPath = '/videos/{videoId}/status'.substring(1)
            .replace('{' + 'videoId' + '}', encodeURIComponent(String(videoId)));
     ...
     * @param metadata Videos can be tagged with metadata tags in key:value pairs. You can search for videos with specific key value pairs using this parameter.
    ...
    public async list(title?: string, tags?: Array<string>, metadata?: { [key: string]: string; }, description?: string, liveStreamId?: string, sortBy?: string, sortOrder?: string, currentPage?: number, pageSize?: number): Promise<VideosListResponse > {
        ...
        if (title !== undefined) {
            urlSearchParams.append("title", ObjectSerializer.serialize(title, "string", ""));
        }
        if (tags !== undefined) {
        ...
        if (liveStreamId !== undefined) {
            urlSearchParams.append("liveStreamId", ObjectSerializer.serialize(liveStreamId, "string", ""));
        }
        if (sortBy !== undefined) {
            urlSearchParams.append("sortBy", ObjectSerializer.serialize(sortBy, "string", ""));
        }
        if (sortOrder !== undefined) {
            urlSearchParams.append("sortOrder", ObjectSerializer.serialize(sortOrder, "string", ""));
        }
     ...
     */
    public async upload(videoId: string, file: File): Promise<Video > {
        const queryParams: QueryOptions = {};
		...
		// Path Params
    	const localVarPath = '/videos/{videoId}/source'.substring(1)
            .replace('{' + 'videoId' + '}', encodeURIComponent(String(videoId)));
     ...
     */
    public async uploadThumbnail(videoId: string, file: File): Promise<Video > {
        const queryParams: QueryOptions = {};
